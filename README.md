# ING
- 4월 12일자 생각치도 못한 서류 합격한 회사들이 존재하게 되어서
- 잠시 미뤄두고, 코테랑 면접이랑 정치기 실기 준비 하다가 4월 19일자부터 다시 시작하기로 결정
- 정처기 실기는 4월 27일 예정이 되어 있으므로, 같이 병행하면서 작업하기로
- TODO : 비밀번호 찾기 페이지를 프론트와 백이랑 독립적인 서버에서 운영한다면? 을 가정하고 findPass 경로에서 이메일과 이름을 인증해서 메일로 인증 코드를 발급한 사람만 접속할 수 있게 인터셉터 또는 필터로 인증 토큰을 새롭게 만들어서 접속 권한이 있는 유저만 접속할 수 있게 설정하고 있는 중 (아마도 토큰 유효 시간도 따로 만들어서 5분? 정도 새롭게 페이지를 구성해야 겠다. - jwt랑 다른 토큰)
